...
analyze.py
----------
Loads results.json and prints a detailed layout-level table plus a
per-decomposition summary. Writes summary.json.
...
def check_compatibility(cz_pairs: list) -> bool:
    for pair in cz_pairs:
        if tuple(sorted(pair)) not in VALID_EDGES:
            return False
    return True
...
def run():
    ...
    # ------------------------------------------------------------------
    # Per-layout detail table
    # ------------------------------------------------------------------
    print(f"\n{'Decomp':<22} {'Layout':<22} {'CZ':<5} {'SX':<5} "
    ...
    for label, entries in results.items():
        for m in entries:
            ...
            status = "YES" if compat else "NO"
            print(
                f"{label:<22} {layout_str:<22} {m['cz']:<5} {m['sx']:<5} "
                f"{m['depth']:<8} {status}"
            )

    # ------------------------------------------------------------------
    # Per-decomposition summary
    # ------------------------------------------------------------------
    summary = {}
    ...
    print(f"Lowest CZ    : {best_cz[0]}    (min_cz={best_cz[1]['min_cz']})")
    print(f"Lowest Depth : {best_depth[0]}    (min_depth={best_depth[1]['min_depth']})")
    print(f"Lowest SX    : {best_sx[0]}    (min_sx={best_sx[1]['min_sx']})")
